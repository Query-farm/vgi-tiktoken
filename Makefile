# vgi-tiktoken worker — dev, test, and lint targets.
#
# Usage:
...
WORKER         ?= $(CURDIR)/target/release/tiktoken-worker
...
test-sql: build
	VGI_TIKTOKEN_WORKER="$(WORKER)" $(SQL_RUNNER) --test-dir "$(TEST_DIR)" "$(TEST_PATTERN)"
