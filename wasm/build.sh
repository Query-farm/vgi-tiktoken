#!/usr/bin/env bash
# Build the browser (`worker:`) build of the tiktoken VGI worker:
# tiktoken-wasm compiled to wasm32-unknown-emscripten, linked by emcc into a
# MODULARIZE'd Web Worker module served over a SharedArrayBuffer ring.
#
...
LIB="target/$TARGET/release/libtiktoken_wasm.a"
...
#
# MALLOC=mimalloc is load-bearing for multithreaded serving: emscripten's default
# dlmalloc serializes every allocation on a global lock, so concurrent serve
# threads convoy on it. mimalloc is thread-caching and scales far better.
#
# STACK_SIZE / DEFAULT_PTHREAD_STACK_SIZE: emscripten defaults to a 64 KiB stack,
# which the Arrow encoders + tokenizer overflow; an overflow surfaces as a bare
# "memory access out of bounds" with no console output. INITIAL_MEMORY must cover
# main stack + every pthread stack up front, or startup stalls trying to grow
# shared memory while the pool is spawning.
echo "==> emcc link"
