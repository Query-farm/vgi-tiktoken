//! The `tiktoken` VGI worker (library).
//!
...
//! the `tiktoken-wasm` crate (the browser build, which serves the same `Worker`
...
/// The catalog name DuckDB sees in `ATTACH 'tiktoken' (TYPE vgi, …)`. Defaults
/// to `tiktoken`, but honors an explicit override so a test harness can rename
/// it. Also exports the variable so downstream SDK code observes the same
/// default.
...
/// metadata. Callers choose the transport — `run()` natively,
