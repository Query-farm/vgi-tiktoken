...
| `TRANSPORT`  | `VGI_TIKTOKEN_WORKER` (the ATTACH LOCATION) | how the worker is launched |
|--------------|------------------------------------------|----------------------------|
| `subprocess` | `…/target/release/tiktoken-worker`          | DuckDB spawns the stdio binary (default) |
| `http`       | `http://127.0.0.1:<port>`                | `tiktoken-worker --http` (auto port; prints `PORT:<n>` on stdout, which the script polls for) |
| `unix`       | `unix:///tmp/tiktoken.<pid>.sock`           | `tiktoken-worker --unix <sock>` (prints `UNIX:<sock>` on stdout + creates the socket; the script waits for both) |
...
3. **Preprocess** — the standalone runner links none of the extensions the
   ...
   through untouched. (The vgi-tiktoken `.test` files already `LOAD vgi;`
   explicitly and use `require-env VGI_TIKTOKEN_WORKER`, so the `require`-rewrite is
...
4. **Run** — [`run-integration.sh`](run-integration.sh) brings up the worker for
   the selected `TRANSPORT` and sets `VGI_TIKTOKEN_WORKER` accordingly, stages the
...
WORKER_BIN="$PWD/target/release/tiktoken-worker" \
TRANSPORT=subprocess \
  ci/run-integration.sh

# HTTP leg: launches `tiktoken-worker --http`, reads PORT:<n>, attaches http://…
HAYBARN_UNITTEST=/path/to/haybarn-unittest WORKER_BIN="$PWD/target/release/tiktoken-worker" \
  TRANSPORT=http ci/run-integration.sh

# Unix leg: launches `tiktoken-worker --unix /tmp/tiktoken.<pid>.sock`, attaches unix://…
HAYBARN_UNITTEST=/path/to/haybarn-unittest WORKER_BIN="$PWD/target/release/tiktoken-worker" \
