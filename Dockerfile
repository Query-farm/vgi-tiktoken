...
#
# Single image that serves every network transport of the `tiktoken` VGI worker:
...
#
# Like vgi-units this worker is STATELESS: the tokenization engine wraps
# `tiktoken-rs`, which BUNDLES the OpenAI BPE encodings into the binary at
# compile time (no network download at runtime), and encoders are built lazily
# and cached for the process lifetime. So there is no /data volume, no model
...
# runtime ships. tiktoken-rs is pure Rust with the encodings vendored, so the
# build needs no network fetch beyond the crate registry.
...
RUN --mount=type=cache,target=/usr/local/cargo/registry \
    ...
    cargo build --release --locked --bin tiktoken-worker \
    && cp target/release/tiktoken-worker /tiktoken-worker
...
ARG SOURCE_URL=https://github.com/Query-farm/vgi-tiktoken
...
LABEL org.opencontainers.image.title="vgi-tiktoken" \
      org.opencontainers.image.description="Exact LLM token counting & token-aware text chunking as a VGI worker for DuckDB/SQL (stdio + HTTP + TCP)" \
...
ENV PORT=8000 \
    ...
    VGI_TIKTOKEN_GIT_COMMIT=${GIT_COMMIT}
...
COPY --from=build --chmod=0755 /tiktoken-worker /usr/local/bin/tiktoken-worker
