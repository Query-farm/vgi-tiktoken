...
#
# Dispatch the single vgi-tiktoken image into one of its transports:
...
#
# The worker is stateless (BPE encodings bundled into the binary), so there is
# no /data to create and no state env to wire — each mode just exec's the binary.
set -e
...
case "${1:-http}" in
  http)
    ...
    exec tiktoken-worker --http "$@"
    ;;
  tcp)
    shift 2>/dev/null || true
    exec tiktoken-worker --tcp "0.0.0.0:${PORT_TCP:-8001}" "$@"
    ;;
  stdio)
    shift 2>/dev/null || true
    exec tiktoken-worker "$@"
    ;;
  *)
    exec "$@"
    ;;
